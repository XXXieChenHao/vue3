  ...
  <div>
    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="type" label="押金分类" align="center">
          <template slot-scope="{row}">{{row | getType}}</template>
        </el-table-column>
        <el-table-column prop="price" label="押金金额（元）" align="center"/>
    ...
    >
      <el-form ref="ruleForm"  :rules="rules" :model="formParam" label-width="120px" size="small">
        <el-form-item label="定额数量: " prop="quota">
           <el-input placeholder="请输入定额数量" v-model.number="formParam.quota" class="input-with-select">
        ...
        <el-form-item label="押金金额: " prop="price">
          <el-input placeholder="请输入押金金额" v-model.number="formParam.price">
            <template slot="append">元</template>
          </el-input>  
        </el-form-item>
        <el-form-item label="压金分类: " prop="type">
          <el-select v-model="formParam.type" placeholder="请选择压金分类">
            <el-option v-for="(val, key, index) in TYPE" :key="index" :label="val" :value="key"></el-option>
          </el-select>
        </el-form-item>
...
import { saveDepositPolicy, pageDepositPolicy, disableDepositPolicy } from '@/api'
const UNIT = {
  '本': '本',
}
...
export default {
  ...
  data() {
      return {
        ...
        TYPE,
        rules: {
          quota: [{required: true, message: '请输入定额数量', trigger: 'blur'}],
          status: [{required: true, message: '请选择规则状态', trigger: 'blur'}],
          price: [{required: true, message: '请输入押金金额', trigger: 'blur'}],
          type: [{required: true, message: '请选择押金分类', trigger: 'blur'}],
  ...
  methods: {
    ...
    handleDel({id}) {
        this.$confirm('此操作将会删除此会员规则, 是否继续?', '提示', {
          ...
          disableDepositPolicy({id}).then(res => {
            this.reset()
            this.getTableData()
          })
        }).catch(() => {
    ...
    // 保存
    savePolicy() {
      saveDepositPolicy(this.formParam).then(res => {
        if(res.code === 200) {
    ...
    getTableData() {
      pageDepositPolicy(this.params).then(res => {
